...
def output_dir_for(data_root, genotype):
    # Deliberately separate from the notebook-generated pooled_results/ folder
    # so the app never overwrites authoritative notebook output.
    path = Path(data_root) / f"pooling_{genotype}" / "app_pooled_results"
...
data_root = Path(st.sidebar.text_input(
    ...
    _default_data_root,
    help=(
        "Base folder for output. Existing pooled_results mode reads from the notebook-generated "
        "pooled_results/ folders. Raw mode writes to app_pooled_results/ (never pooled_results/)."
    ),
))
...
if mode == "Existing pooled_results folders":
    ...
    st.warning(
        "Raw mode writes output CSVs to `app_pooled_results/` inside each genotype folder — "
        "**never** to `pooled_results/` (that folder is reserved for notebook-generated files and will not be touched)."
    )
    ...
    if validation_passed and data_root:
        _critical_files = [
            ("WT",  "WT_all_Events_acinar_clean_yfiltered.csv"),
            ("WT",  "WT_roi_metrics_per_phase.csv"),
            ("WT",  "WT_gmm_clusters_global.csv"),
        ...
        _existing = [
            f"pooling_{geno}/app_pooled_results/{fname}"
            for geno, fname in _critical_files
            if (Path(data_root) / f"pooling_{geno}" / "app_pooled_results" / fname).exists()
        ]
        if _existing:
            st.warning(
                "⚠️ **Running the pipeline will overwrite these existing app output files:**\n\n"
                + "\n".join(f"- `{f}`" for f in _existing)
                + "\n\nMake sure all experiments are loaded in the table above before continuing."
            )
            _overwrite_confirmed = st.checkbox(
                "Yes, I have verified all experiments are in the table and want to overwrite the files listed above.",
                value=False,
                key="overwrite_confirmed",
            )
            if not _overwrite_confirmed:
                st.info("Check the box above to enable the Run button.")

    run = st.button("Run raw pipeline and create tables", type="primary", disabled=not (validation_passed and _overwrite_confirmed))
    if run:
